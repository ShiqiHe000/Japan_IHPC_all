!> @file hybrid_cpu.F90
!> @brief Contains the hybrid version of Laplace.
...
PROGRAM serial
    ...
    IF (VERSION_RUN .eq. "hybrid_small" .and. comm_size /= 2) THEN
        WRITE (*, '(A, I0, A)'), "The small version is meant to be run with 2 MPI processes, not ", comm_size, "."
        CALL MPI_Abort(MPI_COMM_WORLD, -1, ierr)
    ELSE IF (VERSION_RUN .eq. "hybrid_big" .and. comm_size /= 8) THEN
    ...
    CALL initialise_temperatures(temperature, temperature_last)
        ...
        if(my_rank == 0) then
                proc_row(1) = rows/2
                proc_row(2) = rows-proc_row(2)

                proc_col(1:3) = columns/4
                proc_col(4) = columns - columns/4*3

                mpi_bcast(proc_row, 2, mpi_int, 0, mpi_comm_world, ierr)
                mpi_bcast(proc_col, 4, mpi_int, 0, mpi_comm_world, ierr)
        endif
    !///////////////////////////////////
