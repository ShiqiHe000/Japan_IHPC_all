...
void compute( double ** matrix_a, double ** matrix_b, double ** matrix_c, unsigned int matrix_size, unsigned int repetitions  )
{
  ...
  for ( m = 0; m < repetitions; m++ ){
    ...
    gettimeofday( &begin, NULL );
    for ( i = 0; i < matrix_size; i++ ) {
      // TODO give compiler a hint that this loop can be vectorised
      // TODO providing intermediate variable with index offset 
      // TODO int offset_i = 
      for ( k = 0; k < matrix_size; k++ ) {
        // TODO int offset_k = 
        double tmp = (*matrix_a)[i * matrix_size + k];
        for ( j = 0; j < matrix_size; j+=2 ) {
          // TODO use offests in the inner-most loop
          (*matrix_c)[i * matrix_size + j] += tmp * (*matrix_b)[k * matrix_size + j];
          (*matrix_c)[i * matrix_size + j + 1] += tmp * (*matrix_b)[k * matrix_size + j + 1];
        }
      }
    }
    gettimeofday( &end, NULL );
