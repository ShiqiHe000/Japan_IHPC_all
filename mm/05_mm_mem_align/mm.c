...
void allocate( double ** matrix_a, double ** matrix_b, double ** matrix_c, unsigned int matrix_size )
{
  ...
  // allocate memory
  posix_memalign( (void **)matrix_a, 64, matrix_size * matrix_size * sizeof( double * ) );
  posix_memalign( (void **)matrix_b, 64, matrix_size * matrix_size * sizeof( double * ) );
  posix_memalign( (void **)matrix_c, 64, matrix_size * matrix_size * sizeof( double * ) );
  ...
  for ( m = 0; m < repetitions; m++ ){
    ...
    for ( i = 0; i < matrix_size; i++ ) {
      for ( k = 0; k < matrix_size; k++ ) {
        for ( j = 0; j < matrix_size; j++ ) {
          (*matrix_c)[i * matrix_size + j] = (*matrix_c)[i * matrix_size + j] + (*matrix_a)[i * matrix_size + k] * (*matrix_b)[k * matrix_size + j];
        }
      }
    }
    gettimeofday( &end, NULL );
