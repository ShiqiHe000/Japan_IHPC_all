...
void allocate( double ** matrix_a, double ** matrix_b, double ** matrix_c, unsigned int matrix_size )
{
  ...
  *matrix_a = mkl_malloc( matrix_size * matrix_size * sizeof( double* ), 64 ); 
  *matrix_b = mkl_malloc( matrix_size * matrix_size * sizeof( double* ), 64 );
  *matrix_c = mkl_malloc( matrix_size * matrix_size * sizeof( double* ), 64 );
...
void compute( double ** matrix_a, double ** matrix_b, double ** matrix_c, unsigned int matrix_size, unsigned int repetitions )
{
  ...
  for ( k = 0; k < repetitions; k++ ){
    ...
    struct timeval begin, end;
    gettimeofday( &begin, NULL );
    cblas_dgemm(CblasRowMajor, CblasNoTrans, CblasNoTrans, matrix_size, matrix_size, matrix_size, 1.0, *matrix_a, matrix_size, *matrix_b, matrix_size, 0.0, *matrix_c, matrix_size);
   // gsl_blas_dgemm ( CblasNoTrans, CblasNoTrans, 1.0, *matrix_a, *matrix_b, 0.0, *matrix_c );
    gettimeofday( &end, NULL );
