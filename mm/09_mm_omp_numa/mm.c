...
void assign( double ** matrix_a, double ** matrix_b, double ** matrix_c, unsigned int matrix_size )
{
  ...
  #pragma omp parallel
  {
    ...
    unsigned seed = 42 + omp_get_thread_num();
    // assign values
    #pragma omp for private( seed ) schedule( static )
    for ( i = 0; i < matrix_size; i++ ) {
      for ( j = 0; j < matrix_size; j++)  {
        (*matrix_a)[i * matrix_size + j] = ( double ) ( rand_r( &seed ) % 100 );
    ...
    for ( k = 0; k < matrix_size; k += block_size ) {
      for ( j = 0; j < matrix_size; j += block_size ) {
        for ( i_block = i; i_block < i + block_size; i_block++ ) {
          int offset_i_block = i_block * matrix_size;
          for ( k_block = k; k_block < k + block_size; k_block++ ) {
            int offset_k_block = k_block * matrix_size;
            int tmp = (*matrix_a)[offset_i_block + k_block];
            for ( j_block = j; j_block < j + block_size; j_block++ ) {
              (*matrix_c)[offset_i_block + j_block] = (*matrix_c)[offset_i_block + j_block] + tmp * (*matrix_b)[offset_k_block + j_block];
            }
          }
        }
      }
